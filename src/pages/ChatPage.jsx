...
const ChatPage = () => {
  ...
  const loadSession = async (sessionId) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/chat/sessions/${userId}/${sessionId}`);
      const data = await res.json();
      ...
      <div className="flex-1 bg-white dark:bg-zinc-800 rounded-xl border border-gray-100 dark:border-zinc-700 flex flex-col overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-zinc-700">
          <h1 className="text-lg font-medium text-gray-900 dark:text-gray-100">
            {currentSessionId
              ? sessions.find(s => s.sessionId === currentSessionId)?.title || 'Chat Assistant'
              : 'New Conversation'}
          </h1>
...
const ConversationItem = ({ title, messageCount, active, onClick, onDelete }) => (
  <div
    className={`conversation-item px-3 py-2 rounded-lg cursor-pointer transition flex items-center justify-between group ${active
