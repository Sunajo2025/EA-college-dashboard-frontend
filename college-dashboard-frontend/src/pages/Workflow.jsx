...
const Workflow = () => {
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* 🔷 Branded Workflow Animation Section */}
      <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-gray-100 dark:border-zinc-700 p-6">
        ...
        <div className="relative flex items-center justify-between gap-6 overflow-x-auto">
          {/* Trigger Node */}
        ...
        <p className="mt-6 text-sm text-gray-500 dark:text-gray-400">
          This animation represents how active workflows execute events step by step.
        </p>
      </div>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-zinc-700">
            {workflows.map((wf) => (
              <tr key={wf.id}>
                ...
                <td className="px-6 py-4">{wf.trigger}</td>
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-2">
                    {wf.steps.map((s) => (
                      <span
                        key={s}
                        className="px-2 py-1 rounded-full bg-gray-100 dark:bg-zinc-700 text-xs"
                      >
                        {s}
                      </span>
                    ))}
          ...
          <div className="bg-white dark:bg-zinc-900 w-full max-w-2xl rounded-2xl p-6 space-y-4">
            ...
            <ul className="space-y-2">
              {selectedFlow.steps.map((s, i) => (
                <li
                  key={i}
                  className="p-3 rounded-xl bg-gray-50 dark:bg-zinc-800 text-sm"
                >
                  Step {i + 1}: {s}
                </li>
              ))}
...
const FlowNode = ({ title, subtitle }) => (
  <div className="flex flex-col items-center min-w-[160px]">
    <div className="relative">
      <div className="absolute inset-0 rounded-full bg-indigo-500/20 blur-lg animate-pulse" />
      <div className="relative px-4 py-2 rounded-xl bg-indigo-600/10 text-indigo-600 text-sm font-medium">
        {title}
      </div>
    </div>
    <span className="text-xs text-gray-500 mt-2">
